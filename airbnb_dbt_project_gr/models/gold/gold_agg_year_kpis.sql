...
SELECT *, 
ROUND(occupancy_rate - Lead(occupancy_rate,1) OVER(ORDER BY year desc),3) AS occupancy_rate_CHANGE_VS_LAST_YEAR,
ROUND((gross_revenue/NULLIF(Lead(gross_revenue,1) OVER(ORDER BY year desc),0))-1,2) AS gross_revenue_CHANGE_VS_LAST_YEAR,
ROUND((total_profit/NULLIF(Lead(total_profit,1) OVER(ORDER BY year desc),0))-1,2) AS total_profit_CHANGE_VS_LAST_YEAR,
ROUND((revpar/NULLIF(Lead(revpar,1) OVER(ORDER BY year desc),0))-1,2) AS revpar_CHANGE_VS_LAST_YEAR,
ROUND((adr/NULLIF(Lead(adr,1) OVER(ORDER BY year desc),0))-1,2) AS adr_CHANGE_VS_LAST_YEAR,
(avg_guest_rating - Lead(avg_guest_rating,1) OVER(ORDER BY year desc)) AS avg_guest_rating_CHANGE_VS_LAST_YEAR
FROM CTE2 ORDER BY year DESC
